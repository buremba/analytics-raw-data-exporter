...
public class MixpanelEventImporter implements Runnable {
    ...
    public void run() {
        ...
        if(schema != null) {
            try {
                fields = mapper.readValue(schema, new TypeReference<Map<String, Table>>() {});
            } catch (IOException e) {
                throw Throwables.propagate(e);
            }
        } else {
        ...
        if(duration != null) {
            long days = (long) Period.parse(this.duration).getDays();
                ...
                try {
                    mixpanelImporter.importEventsFromMixpanel(entry.getKey(), entry.getValue().rakamCollection, entry.getValue().mapping, finalStart, finalEnd, projectTimezone,
                            (events) -> {
                                ...
                                context.setWriteKey(rakamWriteKey);
                                EventList eventList = new EventList();
                                eventList.setApi(context);
                                eventList.setEvents(events);
                                eventList.setProject(project);
